...
# CONFIGURATION
# ==================================================================================
MODEL_PATH = "./results_v4_distilbert"
MODEL_FILE_LIGHT = 'model_v4_light.pkl'
VEC_FILE_LIGHT = 'vectorizer_v4_light.pkl'
EMOTION_LABELS = [
...
class EmotionInferenceV4:
    def __init__(self, model_path=MODEL_PATH):
        ...
        # 2. Try Loading Lightweight ML Model
            ...
            import pickle
            print(f"Loading Lightweight LinearSVC model...")
            self.model = pickle.load(open(MODEL_FILE_LIGHT, 'rb'))
            self.vectorizer = pickle.load(open(VEC_FILE_LIGHT, 'rb'))
        else:
            print(f"Warning: No models found. System will fail on prediction.")
    ...
    def apply_hybrid_rules(self, text, probs):
        ...
        for emotion, keywords in RULE_TRIGGERS.items():
            if any(k in text_lower for k in keywords):
                # Apply strong boost to the triggered emotion
                boosted_probs[emotion] += 0.5
                triggered = True
                
        # Re-normalize if triggered
        if triggered:
    ...
    def predict(self, text):
        ...
        if self.use_transformer:
            ...
            # Basic preprocessing matching the training
            cleaned = re.sub(r'[^a-z\s]', '', text.lower())
            tokens = cleaned.split()
            processed = ' '.join([stemmer.stem(t) for t in tokens if t not in stopwords.words('english')])
            ...
            for lbl in EMOTION_LABELS:
                if lbl not in prob_dict: prob_dict[lbl] = 0.0
        
        # Apply Hybrid Rule Reinforcement
